...
const RecipeRender = props => {
  const selectedRecipe = () => {
    if (props.selectedRecipe) {
      ...
      const concatArray = ingredientArray.map((ingredient, i) =>
        ingredient.concat(` ${measureArray[i]}`)
      )
      const finalIngredientArray = concatArray.map(ingredient => (
        <p>{ingredient}</p>
      ))

      const splitInstructions = props.selectedRecipe.strInstructions.split(".")
      const mappedInstructions = splitInstructions.map(instruction => (
        <p>{instruction}.</p>
      ))
  ...
  return <RenderStyle>{selectedRecipe()}</RenderStyle>
}
